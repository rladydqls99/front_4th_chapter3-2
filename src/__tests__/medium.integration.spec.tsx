...
it('입력한 새로운 반복 일정 정보에 맞춰 모든 필드가 이벤트 리스트에 정확히 저장된다.', async () => {
  ...
  await saveSchedule(user, {
    ...
    location: '회의실 A',
    category: '업무',
    repeat: { type: 'daily', interval: 2, endDate: '2024-10-17' },
  });

  const eventList = within(screen.getByTestId('event-list'));
  expect(eventList.getAllByText('반복: 2일마다 (종료: 2024-10-17)')).toHaveLength(2);
  expect(eventList.getByText('2024-10-15')).toBeInTheDocument();
...
it('새로 추가한 반복 일정을 수정하는 경우 반복 일정에 관한 표시가 사라진다', async () => {
  ...
  const eventList = within(screen.getByTestId('event-list'));
  expect(await eventList.findByText('반복: 2일마다 (종료: 2024-10-17)')).toBeInTheDocument();
  ...
  expect(eventList.queryByText('반복: 2일마다 (종료: 2024-10-17)')).not.toBeInTheDocument();
});
